...
// esbuild pull the `shiki` package barrel (~250 grammars) into the worker (verified: 15MB→4.9MB
// raw once removed). Output markup matches @shikijs/rehype (dual-theme `.shiki` + --shiki-* vars),
// so globals.css `.shiki` styling is unchanged. Runs AFTER rehype-sanitize (Shiki output is trusted).
export function rehypeShikiVendored() {
  return async (tree: Root): Promise<void> => {
    const highlighter = await getCodeHighlighter();
    const loaded = new Set(highlighter.getLoadedLanguages());

    // `visit` is synchronous; collect targets, then replace (codeToHast is sync once loaded).
